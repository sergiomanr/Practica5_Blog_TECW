...
def menu():
    ...
    respuestas_salida = ['Salir','salir','slir']
    while True:
...
-  ayuda
-  Salir\n""")
      elif respuesta in respuestas_salida:
        break
...
def ordenar():
  ...
  '''
  prods = productos[:]
  # productos.clear()                                   #solo comentar para las primeras formas
  max_prioridad = [i['prioridad'] for i in prods]
  # for e in sorted(set(max_prioridad), reverse=True):  #otra forma de hacerlo
  ...
  for i in productos:
    ...
    productos.clear()
  for e in range(len(comprados)-1):
    for i in range(len(comprados)-1):
      if comprados[i]['prioridad'] < comprados[1+i]['prioridad']:
        key = comprados[i+1]
        comprados[1+i] = comprados[i]
        comprados[i]=key
  else:
    for e in range(len(no_comp)-1):
      for i in range(len(no_comp)-1):
        if no_comp[i]['prioridad'] < no_comp[1+i]['prioridad']:
          key = no_comp[i+1]
          no_comp[1+i] = no_comp[i]
          no_comp[i]=key
  for i in no_comp:
    productos.append(i)
  else:
...
def prueba_manual():
    ...
    insertar('Pan',234,'Alimentación',('arroz a la cubana', 'tortilla'),7)
    insertar('lentejas',3,'Alimentación',('arroz a la cubana', 'tortilla'),7)
    cambiar_estado(2)
    cambiar_estado(3)
    mostrar_productos()
    ordenar()
    # seccion('Ahora ordenada')
    mostrar_productos()
