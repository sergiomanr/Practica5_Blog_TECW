...
productos_unicos = [] #aquí definimos una lista de alimentos que solo se han pedido una vez
...
for alimento in cesta:
    if alimento not in productos_unicos:
        productos_unicos.append(alimento) #aqui añades los alimentos que hemos pedido en general 
        cantidad.append(1) # y pones un uno en cantidad
    else: #si has pedido varias cantidades de un alimento
        index = productos_unicos.index(alimento) # con esto incluimos en productos unicos las cantidades de más
        cantidad_previa = cantidad[index]     
        cantidad[index] = cantidad_previa+1 #y con esto sumas 1 a la cantidad inicial en funcion a cuantas unidades de más has pedido

for alimento in productos_unicos: #y este ultimo for es el que genera el tiket
    precio_unitario = listado_precios.get(alimento, 0.0) #definimos el precio por unidad con un index
    unidades = cantidad[productos_unicos.index(alimento)] # definimos el numero de unidades con otro index
    precio =  precio_unitario * unidades # multiplicamos el numero de unidad por la cantidad de unidades
